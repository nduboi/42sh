...
{
    char **args = NULL;
    char *commands[10] = {"exit", "cd", "setenv", "unsetenv", "env", "echo",
        "history", "alias", "unalias", "jobs"};
    int (*fcts_ptrs[11])(char **, infos_t *) = {my_exit, my_cd, my_setenv,
    ...
    for (; index < 10 && my_strcmp(args[0], commands[index]); index++);
    fcts_ptrs[index](args, infos);
    free(args);
    if (index < 10 && handle_exit_status(GET_STATUS, 0) != 0
        && !infos->isatty)
